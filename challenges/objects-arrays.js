...
// Create a new roar method for the tyrannosaurus. 
// When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
...
const universities = [];
graduates.map( (colleges) => universities.push(colleges.university))
...
*/
const displayNames = [];
zooAnimals.forEach((displayName) => displayNames.push(`Name: ${displayName.animal_name}, Scientific: ${displayName.scientific_name}`))
...
zooAnimals.map((lowerName) => lowCaseAnimalNames.push(`${lowerName.animal_name.toLowerCase()}`));
